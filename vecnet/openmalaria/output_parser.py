    ...
    def _parse_survey_output_file(self, survey_output_file):
        ...
        if survey_output_file is not None:
            ...
            for line in survey_output_file:
                line = line.strip("\r\n")
                data = line.split("\t")
                # The survey number starts from one and corresponds to the survey time point. (Exception: measure 21
                ...
                # for some it holds a mosquito species or a drug identifier.
                ...
                # Value of measure for specified timestep
                value = float(data[3])
...
continuousFileMap = \
        [
            ...
            ("P_df", "30 - 32",
             "The probablity that a mosquito finds a host on a given night, feeds and survives to return to the host-seeking population"),
            ("P_dif", "30 - 32",
             "The probability that a mosquito finds a host on a given night, feeds, gets infected with P. falciparum and survives to return to the host-seeking population"),
            ("alpha", "30.1 - 32",
             "The availability rate of humans to mosquitoes (averaged across human population); units: humans/day (I think)"),
            ("P_B", "30.1 - 32",
             "The probability of a mosquito successfully biting a human after choosing, averaged across humans"),
            ("P_C*P_D", "30.1 - 32",
             "The probability of a mosquito successfully escaping a human and resting after biting, averaged across humans"),
            ("input EIR", "30 - 32",
             "Requested entomological infection rate. This is a fixed periodic value, for comparison with simulated EIR. Units (from schema version 24): inoculations per adult per timestep."),
            ("simulated EIR", "30 - 32",
             "EIR acting on simulated humans. Units: from schema version 26, inoculations per adult per timestep, previously inoculations per person per timestep."),
            ...
            ("human infectiousness", "30 - 32",
             "Infectiousness of humans to mosquitoes, also known as kappa. This is the probability that a mosquito becomes infected at any single feed on a human."),
            ("human age availability", "30 - 32",
             "Mean age-based availability of humans to mosquitoes relative to a human adult (doesn't include any other availability factors, such as vector-model rate or intervention protections)."),
            ("immunity h", "30 - 32",
             "Average of _cumulativeh parameter across humans, which is the cumulative number of infections received since birth"),
            ("immunity Y", "30 - 32",
             "Average (mean) of _cumulativeY parameter across humans, which is the cumulative parasite density since birth"),
            ("median immunity Y", "30 - 32",
             "Average (median) of _cumulativeY parameter across humans, which is the cumulative parasite density since birth"),
            ...
            ("nets owned", "30 - 31",
             "Number of people owning a bed net. Note that people cannot own more than one of a single type of net, so this is usually also the number of nets owned. For version 32, use ITN coverage instead."),
            ("ITN coverage", "32",
             "The number of people owning any type of net divided by the population size. This does not count nets parameterised with the 'GVI' model, only those using the 'ITN' model."),
            ("IRS coverage", "32",
             "The number of people currently protected by any type of IRS divided by the population size. It does not count IRS configured with the 'GVI' model."),
            ("GVI coverage", "32",
            ...
            ("IRS effects", "30.1 - 31",
             "Average effect of IRS on the following three factors: availablity to mosquitoes, preprandial killing, postprandial killing; mean across all humans; both IRS models version 1 and 2; added in schema 30"),
            ("resource availability", "30.3 - 32", "Mean larval resources over a time-step (1/y for these models)"),
            ("requirements availability", "30.3 - 32", "Only for an as-yet unavailable mosquito population dynamics model")
        ]
...
surveyFileMap = \
        [
            ...
            ("annAvgK", "(unused)",
             "Annual Average Kappa. Calculated once a year as sum of human infectiousness weighted by initial EIR for that time of year."),
            ...
            ("Vector_Nv0", "vector species",
             "Number of emerging mosquitoes that survive to the first feeding search per day at this time-step (mosquito emergence rate)."),
            ...
            ("inputEIR", "(unused)",
             "(Previously Vector_EIR_Input.) Input EIR (rate entered into scenario file for vector/non-vector model). Units (schema 24 and later): average inoculations per adult over the time period since the last survey measured in infectious bites per person per time step."),
            ("simulatedEIR", "(unused)",
             "(Previously Vector_EIR_Simulated.) EIR generated by transmission model as measured by inoculations recieved by adults. Units as above (output 35)."),
            None,
            None,
            ...
            ("nSubPopRemovalTooOld", "age group",
             "Number of removals from a sub-population due to expiry of duration of membership (e.g. intervention too old)."),
